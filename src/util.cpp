...
const char * const BITCOIN_CONF_FILENAME = "Xchange.conf";
const char * const BITCOIN_PID_FILENAME = "Xchanged.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "Xchange" is a composite category enabling all Xchange-related debug output
            if(ptrCategory->count(string("Xchange"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\XchangeCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\XchangeCore
    // Mac: ~/Library/Application Support/XchangeCore
    // Unix: ~/.Xchangecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "XchangeCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/XchangeCore";
#else
    // Unix
    return pathRet / ".Xchangecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty Xchange.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override Xchange.conf
