...
UniValue gobject(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in Xchange.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("Xchange.conf", statusObj));
...
UniValue getsuperblockbudget(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            + HelpExampleCli("getsuperblockbudget", "100000")
            + HelpExampleRpc("getsuperblockbudget", "1000o0")
        );
    }
