...
CAmount GetBlockSubsidy(int nBits, int nHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly = false, int nMasternodeCount = 0);
 ...
 * Pruning cannot take place until the longest chain is at least a certain length (50000 on mainnet, 1000 on testnet, 1000 on regtest).
