...
int64_t UpdateTime(CBlockHeader* pblock, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    if(consensusParams.XbufferTSAH<pindexPrev->nHeight)  {
...
// ***TODO*** ScanHash is not yet used in Xchange
//
...
{
    LogPrintf("XchangeMiner -- started\n");
        ...
        while (true) {
            if (chainparams.MiningRequiresPeers()) {
                ...
                do {
                    ...
                    LogPrintf("XchangeMiner -- loop: fvNodesEmpty: %d, IsInitialBlockDownload: %d, IsSynced:%d \n",!fvNodesEmpty, !IsInitialBlockDownload(), masternodeSync.IsSynced());
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("XchangeMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("XchangeMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
            ...
            while (true)
            {
                ...
                if (chainparams.GetConsensus().XbufferTSAH<pindexPrev->nHeight)
                {
    ...
    {
        LogPrintf("XchangeMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("XchangeMiner -- runtime error: %s\n", e.what());
        return;
    }
}
