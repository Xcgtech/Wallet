 ...
 *
 *   // network
 *   CXchangeNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.Xchange.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CXchangeNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CXchangeNetwork(UniValue objIn)
    {
...
// // can be under: XchangeNetwork
// //   -- signature requirements : Key1(User)
// class CXchangeNetworkVariable : public CGovernanceObject
// {
// private:
