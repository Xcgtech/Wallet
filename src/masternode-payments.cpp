...
*
*   Why is this needed?
*   - In Xchange some blocks are superblocks, which output much higher amounts of coins
...
{
    strErrorRet = "";
    if ((sporkManager.GetSporkValue(SPORK_15_Kill_OLD_VERSION)>CLIENT_VERSION_MINOR)){ throw std::runtime_error(strprintf("%s: PLEASE UPDATE YOUR WALLET.%s", __func__,CLIENT_VERSION_MINOR) );}
...
{
    if(fLiteMode) return; // disable all Xchange specific functionality
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f Xchange\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
