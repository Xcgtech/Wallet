...
*Setup instructions for a Gitian build of Xchange Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Xchange
...
- [Building Xchange Core](#building-Xchange-core)
...
Clone the git repositories for Xchange Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/Xchange/Xchange
```
...
in the Xchange Core repository under 'Fetch and build inputs' to install sources which require
...
To build Xchange Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Xchange Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/Xchange/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/Xchange/Xchange
...
For example:
```bash
URL=https://github.com/crowning-/Xchange.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit Xchange=${COMMIT} --url Xchange=${URL} ../Xchange/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit Xchange=${COMMIT} --url Xchange=${URL} ../Xchange/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit Xchange=${COMMIT} --url Xchange=${URL} ../Xchange/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the Xchange git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../Xchange/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/Xchange.git
SIGPATH=/some/root/path/Xchange-detached-sigs.git

./bin/gbuild --url Xchange=${BTCPATH},signature=${SIGPATH} ../Xchange/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/Xchange-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/Xchange-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/Xchange-osx-build.assert
```
...
[Xchange/gitian.sigs](https://github.com/Xchange/gitian.sigs/) repository, or if that's not possible to create a pull
request.
